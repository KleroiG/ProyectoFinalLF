...
import {FiList, FiHash, FiFileText, FiLoader, FiAlertCircle, FiCheckCircle} from 'react-icons/fi';
import TokenInput from '../components/inputs/TokenInput';
import {JWTService, type LexicalAnalysis as LexicalAnalysisResponse} from '../services/jwt.service';
...
export default function LexicalAnalysis() {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            const response = await JWTService.lexicalAnalysis(token);
            ...
            if (response.success) {
                ...
                if (response.errors && response.errors.length > 0) {
                    ...
                    setError(response.error || 'Error en el análisis léxico');
                }
            }
        } catch (err) {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-400 text-lg">
                        Tokenización y análisis de componentes del JWT
                    </p>
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {result && (
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div
                                    className="group bg-slate-900/50 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-xl hover:border-emerald-500/30 transition-all duration-300">
                                    <div className="flex items-center gap-3 mb-4">
                                        <div
                                            className="w-10 h-10 bg-gradient-to-br from-emerald-600/20 to-green-600/20 rounded-lg flex items-center justify-center">
                                            <FiHash className="text-emerald-400 w-5 h-5"/>
                                        </div>
                                        <div>
                                            <h3 className="text-white font-semibold">Partes del Token</h3>
                                            <p className="text-gray-500 text-xs">Segmentos codificados en Base64</p>
                                        </div>
                                    </div>
                                    <div className="space-y-3">
                                        <div className="bg-slate-950/50 border border-white/10 rounded-xl p-4">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span
                                                    className="text-xs text-gray-500">({result.parts.header.length} chars)</span>
                                            </div>
                                            <p className="text-gray-300 text-sm font-mono break-all">{result.parts.header}</p>
                                        </div>
                                        <div className="bg-slate-950/50 border border-white/10 rounded-xl p-4">
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className="text-xs font-semibold text-purple-400">PAYLOAD</span>
                                                <span
                                                    className="text-xs text-gray-500">({result.parts.payload.length} chars)</span>
                                            </div>
                                            <p className="text-gray-300 text-sm font-mono break-all">{result.parts.payload}</p>
                                        </div>
                                        <div className="bg-slate-950/50 border border-white/10 rounded-xl p-4">
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className="text-xs font-semibold text-pink-400">SIGNATURE</span>
                                                <span
                                                    className="text-xs text-gray-500">({result.parts.signature.length} chars)</span>
                                            </div>
                                            <p className="text-gray-300 text-sm font-mono break-all">{result.parts.signature}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-xs">Contenido del encabezado</p>
                                        </div>
                                    </div>
                                    <div className="bg-slate-950/50 border border-white/10 rounded-xl p-4">
                                        <pre
                                            className="text-gray-200 text-sm font-mono whitespace-pre-wrap overflow-x-auto">
                                ...
                                <div
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-xs">{result.headerTokens.length} tokens
                                                identificados</p>
                                        </div>
                                    </div>
                                    <div className="bg-slate-950/50 border border-white/10 rounded-xl p-4">
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-sm">
                                                ...
                                                <tr className="border-b border-white/10">
                                                    <th className="text-left text-gray-400 font-medium py-2 px-3">Posición</th>
                                                ...
                                                <tbody>
                                                {result.headerTokens.map((token, idx) => (
                                                    <tr key={idx}
                                ...
                                <div
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-xs">Contenido de los datos</p>
                                        </div>
                                    </div>
                                    <div className="bg-slate-950/50 border border-white/10 rounded-xl p-4">
                                        <pre
                                            className="text-gray-200 text-sm font-mono whitespace-pre-wrap overflow-x-auto">
                                            {result.payloadDecoded}
                                        </pre>
                                    </div>
                                </div>

                                <div
                                    className="group bg-slate-900/50 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-xl hover:border-pink-500/30 transition-all duration-300">
                                    <div className="flex items-center gap-3 mb-4">
                                        <div
                                            className="w-10 h-10 bg-gradient-to-br from-pink-600/20 to-rose-600/20 rounded-lg flex items-center justify-center">
                                            <FiList className="text-pink-400 w-5 h-5"/>
                                        </div>
                                        <div>
                                            <h3 className="text-white font-semibold">Tokens del Payload</h3>
                                            <p className="text-gray-500 text-xs">{result.payloadTokens.length} tokens
                                                identificados</p>
                                        </div>
                                    </div>
                                    <div className="bg-slate-950/50 border border-white/10 rounded-xl p-4">
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-sm">
                                                ...
                                                <tbody>
                                                {result.payloadTokens.map((token, idx) => (
                                                    <tr key={idx}
                                                        className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                                        <td className="py-2 px-3 text-cyan-400 font-mono">{token.pos}</td>
                                                        <td className="py-2 px-3 text-purple-400 font-semibold">{token.type.toUpperCase()}</td>
                        ...
                        <div
                            ...
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-500 text-xs">Tokenización JWT</p>
                                </div>
                            </div>
